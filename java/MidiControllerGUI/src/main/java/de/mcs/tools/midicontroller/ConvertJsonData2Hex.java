...
public class ConvertJsonData2Hex {
	...
	private static void buildSequenzData(ProgramData programData, byte[] data, SequenceData eventData) throws Exception {
		...
		for (DataData dataData : datas) {
			...
			if (!dataData.getType().equals(DataData.TYPE.PAUSE)) {
	...
	private static void buildSwitches(ByteArrayOutputStream out, ProgramData programData) throws UnsupportedEncodingException, IOException, Exception {
